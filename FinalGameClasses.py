...
# Enum for Dinosaur actions
class DinosaurActions(Enum):
    RUN = "running"
    JUMP = "jumping"
    DUCK = "ducking"

# Dinosaur class with actions and behaviors
class Dinosaur:
    X_POSITION = 80
    ...
    def set_action(self, userInput = None, rl_action = None):
        if (userInput is None and rl_action is None) or (userInput is not None and rl_action is not None): 
            return 
...
def start_environment():
    global clock, player, cloud, background
...
def reset_enviornment(): 
    global clock, player, cloud, background, obstacles 
    clock = pygame.time.Clock() 
    player.__init__() 
    cloud.__init__() 
    background.__init__()
